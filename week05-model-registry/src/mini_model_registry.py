...
def generate_model_card(name, version_id, card_fields, registry_dir):
    ...
    for field in REQUIRED_CARD_FIELDS:
        ...
        if "TODO" in str(value):
            raise ValueError(f"model card field '{field}' still contains a TODO placeholder")
    ...
    if target_stage == "Production":
        ...
        if not os.path.exists(card_path):
            raise GovernanceError(
                ...
                f"no model_card.json for this version"
            )
        f1 = manifest["metrics"]["f1"]
        if f1 < PRODUCTION_F1_THRESHOLD:
            raise GovernanceError(
                f"cannot promote {name} {version_id} to Production: "
                f"f1={f1} is below the {PRODUCTION_F1_THRESHOLD} threshold"
            )

        # At most one Production version at a time: archive whichever other
        # version currently holds Production.
        models_dir = os.path.join(registry_dir, "models", name)
        if os.path.isdir(models_dir):
            for other_id in os.listdir(models_dir):
                if other_id == version_id:
                    continue
                other_path = os.path.join(models_dir, other_id, "manifest.json")
                if not os.path.exists(other_path):
                    continue
                with open(other_path) as f:
                    other = json.load(f)
                if other.get("stage") == "Production":
